...
const CACHE_KEY = 'joel-published-collection-v1';
...
async function update() {
  ...
  status.textContent='Loading the latest collection…';
  try {
    const manifest=await loadManifest();
    populate(manifest);
    status.textContent=`Collection updated ${timestamp(manifest.generatedAt)} · Syncs daily`;
    try { localStorage.setItem(CACHE_KEY,JSON.stringify(manifest)); } catch { /* Storage is optional. */ }
  } catch {
    status.textContent=projects.length ? 'Showing your saved collection. Try Refresh again later.' : 'Could not load projects. Try Refresh or visit GitHub.';
    if(!projects.length)document.querySelector('#empty').hidden=false;
...
try {
  ...
  if(saved)populate(validateManifest(JSON.parse(saved)));
} catch { /* A missing or invalid cache is recovered from the published manifest. */ }
await update();
